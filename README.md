...
![status](https://img.shields.io/badge/vers%C3%A3o-0.2.37-orange) ![tests](https://img.shields.io/badge/testes-415%20passando-brightgreen)

## O que já faz (v0.2.37)
...
- **Dimensionamento NBR 6118**: vigas (flexão com **MESA COLABORANTE §14.6.2.2** — seção T automática nos positivos quando há laje colada ao vão, bf = bw + mín(0,5·b2; 0,10·a) por lado com a por continuidade, LN na mesa ou decomposição mesa+alma — economia real de armadura + cisalhamento + **torção §17.5** +
  ...
  **escadas RETAS, EM L e EM U** (lance como laje inclinada; L/U: lance + patamar contínuos com Mmax exato onde V = 0, nota da armadura da dobra contra empuxo ao vazio, dois lances no 3D) e
...
- **ALVENARIA ESTRUTURAL (NBR 16868-1:2020) — Fase 1**: parede estrutural como
  ...
  que vai p/ a obra), paredes no 3D; **Fase 2**: ABERTURAS (portas/janelas criando grupos isolados com concentração de carga pelas vergas) e **CONTRAVENTAMENTO AO VENTO** (distribuição por rigidez I = t·L³, cisalhamento fvk = 0,15+0,5·σ ≤ 1,4 MPa, flexocompressão com tração de borda ⇒ graute + armadura calculada)
